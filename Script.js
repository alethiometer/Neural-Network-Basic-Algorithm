// See Script Overview for more extensive explanation of these methods.
...
function introducingNeuralNetwork() {
  console.log("Hello!");
  console.log(" ");
  console.log("This program is meant to simulate a very basic neural network. But what is a neural network?");
  console.log(" ");
  console.log("Great question! A neural network works much like the neurons of the human brain and nervous system; sensory input whether by taste, touch, sound, sight, or smell stimulates a certain body response. In a similar way, a computational neural network receives certain input and produces a desired.");
  console.log(" ");
  console.log("However, there is one fundamental difference. Our sensory input often renders an involuntary body response, whereas the computer has no idea how to respond to certain input! That's why we have machine learning, which we use to train the computer so we arrive at a desired output.");
  console.log(" ");
  console.log("At the heart of a neural network lies three types of layers: the input, hidden, and output layers. These are organized in that order from left to right. Each of these layers contains several \"neurons,\" and for this program, each layer has two neurons.");
  console.log(" ");
  console.log("Each neuron of the input layer is linked to each neuron of the hidden layer, and each neuron of the hidden layer is linked to each neuoron of the output layer. But what exactly do we mean by link?");
  console.log(" ");
  console.log("A link is represented by a numerical value we call a weight. In this program, the weights are initially set to somewhat random values. Now how do the layers and weights interact with each other?");
  console.log(" ");
  console.log("The layers and weights interact in two primary parts: forward propagation and back propagation.");
  console.log(" ");
  console.log("In forward propagation we manipulate the input values with the weights (between the input and hidden layers) to produce hidden values. Then we manipulate the hidden values with the weights (between the hidden and output layers) to produce output values.");
  console.log(" ");
  console.log("When we reach our outputs, we compare our results with the desired outputs, which we can call target outputs. For this program, our inputs are 0.05 and 0.10, and the target outputs are 0.01 and 0.99, heuristically close to 0 and 1, or binary true and false. In comparing them, we come up with a number we call the Total Error, which represents how far off our actual outputs are from the targets.");
  console.log(" ");
  console.log("Then we move on to backward propagation, where we manipulate now manipulate the weight values to ensure the results are closer to the target.");
  console.log(" ");
  console.log("This program implements a number of standard formulas like the logistical formula, the total error formula, the delta rule, and the partial derivative, as well as standard constant values such as the bias and the learning rate. This is complex Calculus even I only learned over the past two years of high school. However, understanding how to apply these formulas and the mathematical theorems that allow us to arrive at each vastly deepens our understanding of neural networks.");
  console.log(" ");
  console.log("Many of the important formulas are described in their own unique function, for this program is meant to illustrate the fomulaic applications and show that neural networks aren't as abstract or daunting as we think. Though this program is very very simple relative to the complex applications of neural networks used in the professional world, this acts as a theoretical exercise so that high schoolers like me may understand the basic concepts.");
  console.log(" ");
  console.log("Stay tuned till the end, and see how after 10,000 trials we nearly reach our target output :)");
}
